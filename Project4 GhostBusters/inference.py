...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            position.append((i, j))
    variableDomainsDict[PAC] = position
    variableDomainsDict[GHOST0] = position
    variableDomainsDict[GHOST1] = position
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        sample = None
        for key in self.keys():
            if self[key] <= randomNumber:
                randomNumber -= self[key]
                sample = key
            else:
                return key
        return sample
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            self.beliefs[position] *= self.getObservationProb(observation, gameState.getPacmanPosition(),
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            for newPos in self.allPositions:
                newDist[newPos] += newPosDist[newPos] * self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        numOfParticle = int(self.numParticles / len(self.legalPositions))
        for pos in self.legalPositions:
            for _ in range(numOfParticle):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in oldBelief:
            newPosDist = self.getPositionDistribution(gameState, particle)
            for newPos in newPosDist:
                newBelief[newPos] += oldBelief[particle] * newPosDist[newPos]
        self.particles = [newBelief.sample() for _ in range(self.numParticles)]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        particlesDomain = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(particlesDomain)
        size = len(particlesDomain)
        for i in range(self.numParticles):
            self.particles.append(particlesDomain[i % size])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in range(self.numGhosts):
            for position in self.legalPositions:
                ghostPosDict[(i, position)] = self.getObservationProb(observation[i], gameState.getPacmanPosition(),
                                                                      position, self.getJailPosition(i))
        for ghostPositions in belief:
            for i in range(self.numGhosts):
                belief[ghostPositions] *= ghostPosDict[(i, ghostPositions[i])]
            Sum += belief[ghostPositions]
        if Sum == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [belief.sample() for _ in range(self.numParticles)]
