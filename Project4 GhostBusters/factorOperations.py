...
def joinFactors(factors: List[Factor]):
    ...
    for factor in factors:
        ...
        for conditionedVar in factor.conditionedVariables():
            if conditionedVar in Unconditioned or conditionedVar in Conditioned:
                continue
            Conditioned.append(conditionedVar)
    result = Factor(Unconditioned, Conditioned, dict)

    for assignmentDict in result.getAllPossibleAssignmentDicts():
        probability = 1
        for factor in factors:
            probability *= factor.getProbability(assignmentDict)
        result.setProbability(assignmentDict, probability)
    return result
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        dict = factor.variableDomainsDict()
        result = Factor(unconditionedVars, conditionedVars, dict)

        for assignmentDict in factor.getAllPossibleAssignmentDicts():
            result.setProbability(assignmentDict, result.getProbability(assignmentDict) + factor.getProbability(assignmentDict))
        return result
